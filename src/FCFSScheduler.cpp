            ...
            for (int i = 0; i < numCores; i++) {
                if (cores[i] && !cores[i]->IsFinished() && cores[i]->GetTotalInstructions() > 0) {
                    ...
                    if (cores[i]->IsFinished()) {
                        if (memory) {
                            memory->FF_DetachProcessFromMemory(cores[i].get());
                        }
                        doneQueue.push(std::move(cores[i]));
                        if (!readyQueue.empty()) {
                            auto nextProcess = std::move(readyQueue.front());
                            readyQueue.pop();
            ...
            if (enableBatchFlag && batchProcessFrequency > 0 && cpuCycles % batchProcessFrequency == 0) {
                ...
                for (int i = 0; i < numCores; ++i) {
                    if (!cores[i]) {
                        ...
                        if (memory) {
                            memory->FF_AssignProcessToFrame(cores[i].get(), cores[i]->GetTotalInstructions());
                        }
                        currentPidInc++;
                        assignedToCore = true;
                        break;
                ...
                if (!assignedToCore) {
                    string processName = "screen_" + padNumberFCFS(currentPidInc, 2);
                    unsigned int instructions = minInstructions + (rand() % (maxInstructions - minInstructions + 1));
                    auto newProcess = std::make_unique<Screen>(currentPidInc, instructions, getCurrentTimestamp(), processName);
                    if (memory) {
                        memory->FF_AssignProcessToFrame(newProcess.get(), newProcess->GetTotalInstructions());
                    }
...
void FCFSScheduler::CreateProcess(bool isBatch, const string& userProvidedName) {
    ...
    string processName = isBatch ? "screen_" + padNumberFCFS(currentPidInc, 2) : userProvidedName;
    unsigned int instructions = minInstructions + (rand() % (maxInstructions - minInstructions + 1));
    auto newProcess = std::make_unique<Screen>(currentPidInc, instructions, getCurrentTimestamp(), processName);
...
Screen* FCFSScheduler::GetProcessByName(const std::string& name) {
    lock_guard<mutex> lock(schedulerMutex);
